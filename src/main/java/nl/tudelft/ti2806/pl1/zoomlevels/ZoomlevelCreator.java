...
public final class ZoomlevelCreator {
	...
	public Graph createGraph(final int threshold) {
		...
		ret = InDelCollapser.collapseInsertions(graph.getInsMutations(), ret, threshold);
		ret = InDelCollapser.collapseDeletions(graph.getDelMutations(), ret, threshold);
		ret = ComplexCollapser.collapseComplexMutations(graph.getComplexMutations(), ret, threshold);
		ret = PointCollapser.collapseNodes(graph.getPointMutations(), ret, threshold, graph.getSelected());
		if (threshold != 0) {
